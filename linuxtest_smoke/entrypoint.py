...
test_result_dir=project_dir+"/results/"
...
def confirm_actual_session_id():
  ...
  db_select_tuple=db_select.db_select(my_stmt)
  print("DEBUG: db_select_tuple[0]:")
  print(db_select_tuple[0])
  print("DEBUG: db_select_tuple[1]:")
  for i in db_select_tuple[1]:
    print(i)
  iterated_results=db_select_tuple[1]
...
def create_session():
  ...
  iterated_results=confirm_actual_session_id()
  for i in iterated_results:
    print(i)
...
def run_test():
  ...
  if int(section_I_want) < 10:
    this_results_filename="0"+section_I_want+"_03.txt"
  else:
    this_results_filename=section_I_want+"_03.txt"
  print("DEBUG: this_results_filename is: " + this_results_filename)
  this_results_file=os.path.join(test_result_dir,this_results_filename)
  print("DEBUG: this_results_file is: " + this_results_file)
  ...
  # instantiate incorrect_questions as list
  incorrect_questions = list()
  # instantiate unknown_questions as list
...
#  print(incorrect_questions)
  ...
  for q_id in question_ids:
    ...
    print("")
  #  print(q_id)
    # get a random seed from imported get_random()
    this_rand=get_random()
  #DEBUG
  #  print("DEBUG: this_rand = %r") % this_rand
  #  print(this_rand)
  ...
  #  print("DEBUG: incorrect_4 is: %r") % incorrect_4
    ...
    # NOTE: 4 rstrip functions allow a line ending with [thing] to be printed as
    #       [thing] rather than as [thing - TO-DO - WHY IS THIS THIS WAY?
...
##print(unknown_questions)
  output=''
  output=output+"RUN: section score for section id %r:\n" % section_I_want
  output=output+"-- section_q_count: " + str(section_q_count) + "\n"
  output=output+"-- correct_a_count: " + str(correct_a_count) + "\n"
  output=output+"-- incorrect_a_count: " + str(incorrect_a_count) + "\n"
  output=output+"-- unknown_count: " + str(unknown_a_count) + "\n"
  output=output+"---- incorrect_questions:\n"
  output=output+str(incorrect_questions) + "\n"
  output=output+"---- unknown_questions:\n"
  output=output+str(unknown_questions) + "\n"
  print(output)
  #print("DEBUG: creating resultsfile " + this_results_file)
  f = open(this_results_file,'x')
  f.close()
  #print("DEBUG: opening resultsfile " + this_results_file)
  f = open(this_results_file,"w+")
  f.write(output)
  f.close
  print("RUN: wrote output to " + this_results_file)
  #print("DEBUG: section_answer_tuple_list:" + section_answer_tuple_list)
...
def main():
  ...
  while menu_option == '':
    ...
    if menu_option == "show_stats" or menu_option == "5":
      show_stat()
      #return 0
    if menu_option == "list_sections" or menu_option == "6":
      input_sections.list_sections()
      #return 0
